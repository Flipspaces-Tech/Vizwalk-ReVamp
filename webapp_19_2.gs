...
const SHEET_NAME = 'Featured Projects Page'; // Tab name for Build→AppId lookup
...
function normalizeImageUrl(raw) {
  ...
  // ✅ handle glued URLs by extracting the last valid one
  raw = extractLastUrl(raw);

  return raw;
}


// Extract the last URL from a glued string like:
// "http://ec2...amazonaws.comhttps://s3.../file.png"
function extractLastUrl(raw) {
  raw = String(raw || '').trim();
  if (!raw) return '';

  // Split whenever a new URL begins (works for glued "http...https..." strings)
  const parts = raw
    .split(/(?=https?:\/\/)/g)
    .map(s => String(s).trim())
    .filter(Boolean);
...
function doGet(e) {
  ...
  // ... keep your existing actions: ping, getappid, proxyget, listscreenshots, etc.

  if (action === "demovideos") {
    const sheetId = String(e.parameter.sheetId || SHEET_ID).trim(); // use your existing SHEET_ID fallback
    const tabName = String(e.parameter.tab || "Demo Videos Data For Website").trim();
    ...
    for (let r = 1; r < values.length; r++) {
      ...
      // Only Active rows
      const status = String(obj["Status"] || "").toLowerCase().trim();
      if (status !== "active") continue;

      rows.push({
  status: obj["Status"],
  videoName: obj["Videos Name"] || "",
  projectSlot: obj["Project Slot Name"] || "",
  sbu: obj["SBU"] || "",
  areaSqft: obj["Area (SqFt)"] || "",
  constructionType: obj["Construction Type"] || "",
  thumbnailUrl: obj["Thumbnail_URL"] || "",

  // ✅ Project Showcase button
  youtubeUrl: obj["Unlisted Youtube Video Link"] || "",

  // ✅ Interactive Video button
  vizwalkDemoUrl: obj["Unlisted Youtube Vizwalk Demo Video Link"] || "",
});
