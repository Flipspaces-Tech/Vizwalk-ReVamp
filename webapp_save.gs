...
function doGet(e) {
  try {
    ...
    if (action === 'getappid') {
      ...
      const verQuery   = String(p.ver   || '').trim();   // 👈 from ?ver=
      ...
      const colVersion  = findCol(header, 'Build Version');   // 👈 new column
      ...
      const qVer   = norm(verQuery);  // may be empty string

      let exactMatch = null;   // build + version
      let buildOnly  = null;   // build match without version (fallback)

      for (var i = 0; i < values.length; i++) {
        var row       = values[i];
        var bRaw      = row[colBuild];
        var b         = norm(bRaw);
        var slot      = String(row[colSlot] || '').trim();
        var status    = colStatus >= 0 ? norm(row[colStatus]) : '';
        var vRaw      = colVersion >= 0 ? row[colVersion] : '';
        var v         = norm(vRaw);

        if (!slot) continue;
        if (!(b === qBuild || b.indexOf(qBuild) >= 0)) continue;

        // only consider Active rows if Status column exists
        if (colStatus >= 0 && status && status !== 'active') continue;

        // If a version is provided and we have a Build Version column, prefer exact match
        if (qVer && colVersion >= 0) {
          if (v === qVer) {
            ...
            // remember first row with this build name (wrong version)
            buildOnly = {
              appId:  slot,
              status: status,
              build:  bRaw,
          ...
          // no version specified → first active row with this build
          buildOnly = buildOnly || {
            appId:  slot,
            status: status,
            build:  bRaw,
      ...
      if (exactMatch) {
        return out({
          ...
          build: exactMatch.build,
          version: exactMatch.version
        });
      }

      // version was provided but no exact row found
      if (qVer) {
        if (buildOnly) {
          return out({
            ok: false,
            ...
            closest: {
              build:   buildOnly.build,
              version: buildOnly.version,
              appId:   buildOnly.appId
            }
          });
        }
        return out({
      ...
      // no version param → fallback to first build match
      if (buildOnly) {
        return out({
          ok: true,
          appId: buildOnly.appId,
          status: buildOnly.status,
          build: buildOnly.build,
          version: buildOnly.version
        });
      }
